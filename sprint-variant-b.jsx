// Variant B — Elegante consultora / editorial
// Más blanco, serif dominante (DM Serif + EB Garamond), azul profundo, sensación premium

function SprintVariantB() {
  const { track } = useSprintTrack();
  ...
  return (
    <nav style={{ position: 'sticky', top: 0, zIndex: 60, background: scrolled ? 'rgba(245,242,236,0.92)' : 'rgba(245,242,236,0.7)', backdropFilter: 'blur(14px)', borderBottom: `1px solid ${scrolled ? 'rgba(0,0,0,0.08)' : 'transparent'}`, transition: 'all 250ms' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '18px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 18 }}>
        <a href="Home.html" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none', color: '#0A0A0A' }}>
          <span className="serif" style={{ fontSize: 22, letterSpacing: '-0.02em' }}>adsBigger</span>
          <sup style={{ fontSize: 10, color: '#888' }}>™</sup>
        </a>
        <div style={{ display: 'flex', gap: 28, alignItems: 'center' }}>
          <a href="Home.html" style={{ color: '#444', textDecoration: 'none', fontSize: 14 }}>Sistema</a>
          <a href="Kit.html" style={{ color: '#444', textDecoration: 'none', fontSize: 14 }}>Kit · $97</a>
          <a href="#top" style={{ color: '#0A0A0A', textDecoration: 'none', fontSize: 14, fontWeight: 500 }}>Sprint</a>
        </div>
        <a href="#aplicar" onClick={() => onCTA('Nav_Apply')} style={{ background: '#0A0A0A', color: '#fff', padding: '12px 22px', borderRadius: 99, textDecoration: 'none', fontSize: 13, fontWeight: 500, fontFamily: 'DM Sans' }}>
          Aplicar al Sprint
        </a>
      </div>
    </nav>
    ...
    <section id="timeline" style={{ background: '#F5F2EC', padding: '140px 40px', borderTop: '1px solid rgba(0,0,0,0.08)' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto' }}>
        ...
        <div style={{ marginTop: 64, display: 'flex', flexDirection: 'column', gap: 0 }}>
          {phases.map((p, i) =>
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '120px 1fr 2fr', gap: 40, padding: '40px 0', borderBottom: '1px solid rgba(0,0,0,0.12)', alignItems: 'start' }}>
              <div>
              ...
              <div>
                <ul style={{ listStyle: 'none', padding: 0, margin: 0, columns: 2, columnGap: 28 }}>
                  {p.items.map((x, j) =>
        ...
        {/* Bonus editorial */}
        <div style={{ marginTop: 48, background: '#0A0A0A', color: '#F5F2EC', borderRadius: 6, padding: '48px 48px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48 }}>
          <div>
          ...
          <div style={{ borderLeft: '1px solid rgba(255,255,255,0.15)', paddingLeft: 48 }}>
...
function ApplyB({ track }) {
  return (
    <section id="aplicar" style={{ background: '#0A0A0A', color: '#fff', padding: '140px 40px' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: '1.15fr 1fr', gap: 64, alignItems: 'start' }}>
        <div>
          ...
          <div style={{ marginTop: 32, display: 'flex', flexDirection: 'column', gap: 14, maxWidth: 520 }}>
            {['Respondemos por WhatsApp en menos de 2 horas', 'No hay llamada de ventas — conversación directa', 'Reservamos cupo solo si hay match real', 'Pago único $5M COP / USD $1.270'].map((t, i) =>
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14, fontSize: 15, color: '#ddd' }}>
                <span style={{ width: 6, height: 6, borderRadius: 99, background: '#F5C518' }} />{t}
              </div>)}
          </div>
        </div>
        <SprintApplyForm variant="B" track={track} />
      </div>
    </section>
  );
}
    ...
    <footer style={{ background: '#0A0A0A', color: '#888', padding: '48px 40px 36px', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
      <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 22, fontSize: 13 }}>
          <a href="Home.html" style={{ color: '#888', textDecoration: 'none' }}>Sistema</a>
          <a href="Kit.html" style={{ color: '#888', textDecoration: 'none' }}>Kit · $97</a>
...
Object.assign(window, { SprintVariantB });
